...
const Page = () => {
    ...
    const fetchAcceptMessage = useCallback(async () =>{
        ...
        try {
           ...
           setValue('acceptMessage', response.data.isAcceptingMessages ?? false) //or Boolean(response.data.isAcceptingMEssages) or isAcceptingMessages!
    ...
    const handleSwitchChange = async()=>{
        try {
            ...
            const response = await axios.post<ApiResponse>('/api/accept-message',{
                acceptMessages:newValue//if true make it false and viceversa to the acceptmessages
            })
...
useEffect(() => {
  setBaseUrl(`${window.location.protocol}//${window.location.host}`)
    ...
    return(
        <div className="my-8 mx-4 md:mx-8 g:mx-auto p-6 bg-white rounded w-full max-w-6xl">
            ...
            <div className="mb-4">
                ...
                onClick={(e)=>{
                    ...
                    {isLoading ? (
                        <Loader2 className="h-4 w-4 animated-spin"></Loader2> 
                    ):(
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                    {messages.length > 0 ? (
                        messages.map((message, index)=>(
                            ...
                            message={message}
                            onMessageDelete={handleDeleteMessage}></MessageCard>
                        ))
                    ):(
